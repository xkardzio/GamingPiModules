...
class LedSerialHandler:
    ...
    def __init__(self, port, baudrate=9600, timeout=0, init_time=INIT_TIME):
        ...
        init_thread = threading.Thread(
            target=self.serial_init, args=(port, baudrate, timeout, init_time)
        )
        init_thread.start()
    ...
    def init(self):
        ...
        if (
            ...
            == LedSerialHandler.Response.INIT_SUCCESS.value
        ):
            return True
        else:
            return False
    ...
    def _process_queue(self):
        while self.running or not self.queue.empty():
            try:
                if self.serial_port is not None:
                    message, read_bytes_count, response_queue = self.queue.get(
                        timeout=0
                    )
